...
struct AdCandidate {
    ...
    double ecpm;                             ///< eCPM = bid_price * ctr * 1000

    // 预算和消费信息
    std::optional<double> remaining_budget;  ///< 剩余预算(元)
    std::optional<uint64_t> daily_spend;     ///< 今日已消费(微元)

    // 扩展字段
    std::optional<std::string> deal_id;      ///< Deal ID (PMP交易)
    std::optional<double> quality_score;     ///< 质量分
